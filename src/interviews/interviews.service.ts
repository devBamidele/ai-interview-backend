...
export class InterviewsService {
  ...
  async analyzeInterview(
    ...
    try {
      ...
      this.logger.log(`Interview record created with ID: ${interview._id}`);

      // Perform AI analysis
      try {
        const aiAnalysis = await this.aiService.analyzeInterview(
          dto.sessionData,
        );
        ...
        throw new InternalServerErrorException('Failed to analyze interview');
      }
    } catch (error: any) {
      logError(this.logger, 'Failed to analyze interview', error);
  ...
  async getInterview(interviewId: string): Promise<GetInterviewResponse> {
    ...
    try {
      ...
      logError(this.logger, 'Failed to fetch interview', error);

      throw new InternalServerErrorException('Failed to fetch interview');
    }
  }
}
