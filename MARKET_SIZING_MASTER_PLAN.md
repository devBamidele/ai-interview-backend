...
#### What NOT to Score:
- ❌ Whether they used a specific named framework
- ❌ Exact accuracy of final number
- ❌ Speed of completion (10 min is the target, not a race)
- ❌ Confidence scores (too subjective)
- ❌ "Answer Reasonableness" as separate category (covered in Sanity Check)

#### What TO Score:
- ✅ Process quality over answer precision
- ✅ Clear thinking articulated out loud
- ✅ Logical structure and MECE segmentation
- ✅ Realistic assumptions with justification
- ✅ Step-by-step math shown and verbalized
- ✅ Explicit sanity check performed
...
- **Results Screen:**
  ...
  📊 Detailed Evaluation (MBB Criteria):

  Structured Problem-Solving: 4/5 (Very Good) - 30% weight
  ├─ Clear top-down framework articulated
  ├─ MECE segmentation applied
  └─ Could improve: Ask more clarifying questions upfront

  Business Judgment & Assumptions: 3/5 (Good) - 25% weight
  ├─ Assumptions were reasonable
  ├─ Most assumptions justified
  └─ Could improve: Articulate WHY each assumption is reasonable

  Quantitative Skills: 3/5 (Good) - 20% weight
  ├─ Math was mostly accurate
  ├─ Showed some intermediate steps
  └─ Could improve: Verbalize ALL calculations out loud

  Communication: 4/5 (Very Good) - 15% weight
  ├─ Clear explanations (Pace: 142 WPM)
  ├─ Low filler words (8 total)
  └─ Good structure throughout

  Sanity Check: 2/5 (Adequate) - 10% weight
  ├─ Performed basic reality check
  └─ Could improve: Verbalize sanity check reasoning explicitly
  ...
  ✨ What You Did Well:
  • Clear framework articulation at start
  • MECE segmentation (no overlaps)
...
```
1. User starts interview → Voice Agent asks behavioral questions
